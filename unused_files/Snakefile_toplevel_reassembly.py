...
rule extract_reads_bulk:
  ...
  params:
    ...
    qos="normal",
    time="12:00:00", # parameters.ix['reassembly_bowtie2_time','entry'],
    mem_per_core="6G",
    partition=parameters.ix['subsample_bowtie2_partition','entry'],
    mem="128000" # parameters.ix['subsample_bowtie2_memory','entry']
  threads: 10 # int(parameters.ix['subsample_bowtie2_thread','entry'])
  version: "1.0"
  run:
    # Manage files and obtain scratch location
    scratch = os.environ["LOCAL_SCRATCH"]
...
rule extract_reads_minimeta:
  ...
  output: # output files could be temp
    temp("Genome_groups/miniMetaReads_{subsample}_GenomeCluster_{genome}_P1.fastq"),
    temp("Genome_groups/miniMetaReads_{subsample}_GenomeCluster_{genome}_P2.fastq"),
    temp("Genome_groups/miniMetaReads_{subsample}_GenomeCluster_{genome}_S.fastq")
  params:
    name="extract_reads_minimeta",
    qos="normal",
...
rule assemble_genome_with_bulk:
  ...
  run:
    ...
    shell("""
      ...
      quast.py --gene-finding --glimmer -m {params.contig_thresh} -t {threads} -o $spades_output_dir/quast_output {output[1]}
...
rule assemble_genome_minimeta_only:
  ...
  run:
    ...
    shell("""
      ...
      cp $spades_output_dir/scaffolds.fasta {output[2]}
      source activate {python2_env}
      # do not set minimun contig threshold, default is 500bp
      quast.py --gene-finding --glimmer -t {threads} -o $spades_output_dir/quast_output {output[1]}
      cp $spades_output_dir/quast_output/report.txt {output[3]}
      date; source deactivate
      """)
    assert(file_empty(output)),"Either the contig or scaffold file is empty."
...
rule extract_reads_minimeta_allSubSamples:
  input:
    ...
    "Genome_groups/genome_cluster_{genome}.rev.2.bt2l"
  output: # output files could be temp
    temp(expand("Genome_groups/miniMetaReads_{subsample}_GenomeCluster_{{genome}}_P1.fastq", subsample=subsampleIDs)),
    temp(expand("Genome_groups/miniMetaReads_{subsample}_GenomeCluster_{{genome}}_P2.fastq", subsample=subsampleIDs)),
    temp(expand("Genome_groups/miniMetaReads_{subsample}_GenomeCluster_{{genome}}_S.fastq", subsample=subsampleIDs))
  params:
  ...
  run:
    ...
    for i in range(numSubSamples):
      p1 = input[0 * numSubSamples + i]
      p2 = input[1 * numSubSamples + i]
      s1 = input[2 * numSubSamples + i]
      s2 = input[3 * numSubSamples + i]
      out_p1 = output[0 * numSubSamples + i]
      out_p2 = output[1 * numSubSamples + i]
      out_s = output[2 * numSubSamples + i]
      shell(""
        date; source activate {python3_env}
        basename=$(echo {input[-1]} | cut -d. -f1)
        echo $basename
        cat {s1} {s2} > {scratch}/single_reads.fastq
        echo 'Using end-to-end alignment to remove reads'
        bowtie2 --time --phred33 --very-sensitive-local -I 100 -X 2000 -p {threads} --al {scratch}/alignedSingle --al-conc {scratch}/alignedPaired -x $basename -1 {p1} -2 {p2} -U {scratch}/single_reads.fastq -S {scratch}/alignResults.sam
        mv {scratch}/alignedPaired.1 {out_p1}
        mv {scratch}/alignedPaired.2 {out_p2}
        mv {scratch}/alignedSingle {out_s}
        source deactivate
        "")
"""
